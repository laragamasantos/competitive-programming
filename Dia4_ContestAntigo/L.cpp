...
int32_t main(){ sws;
    ...
    loop(a, 0, entrada.size()){
        tempos.pb(mp(entrada[a], a+1));
    }

    stable_sort(all(tempos));

    loop(j, 1, 4){
        cout << fixed << setprecision(2);
        cout << j << "o. lugar: competidor " << tempos[0+aux].second <<" - " << tempos[0+aux].first <<" segundos" << endl;
        aux++;
    }
